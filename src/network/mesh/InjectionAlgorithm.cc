...
InjectionAlgorithm* InjectionAlgorithm::create(
  ...
  InjectionAlgorithm* ia =
      factory::ObjectFactory<InjectionAlgorithm, MESH_INJECTIONALGORITHM_ARGS>::
          create(algorithm, _name, _parent, _interface, _baseVc, _numVcs,
                 _inputPc, _settings);

  // check that the factory had this type
  if (ia == nullptr) {
    fprintf(stderr, "invalid Mesh injection algorithm: %s\n",
            algorithm.c_str());
    assert(false);
  }
  return ia;
